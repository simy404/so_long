...
void render_map(t_game* game)
{
	...
	while (game->map->map[col])
	{
		...
		{
			if (game->map->map[col][row] != '0')
			{
				img = get_image_by_tile(game->graphics, game->map->map[col][row]);
				mlx_put_image_to_window(game->graphics->mlx, game->graphics->mlx_win, img, row * TILE_SIZE, col * TILE_SIZE);
			}
			row++;
		}
		col++;
...
int	create_images(t_graphics* graphics)
{
	...
	graphics->exit_img = load_image(graphics, "./assets/dgs.xpm");
...
int	create_window(t_game* game)
{
	...
	game->graphics->mlx_win = mlx_new_window(game->graphics->mlx, game->map->map_row_count * TILE_SIZE,
		game->map->map_col_count * TILE_SIZE, "so_long");
